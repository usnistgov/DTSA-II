...
public class MainFrame
   ...
   static private class RecentFile
      extends
      File {
   ...
   static private class RecentPyModel
      extends
      DefaultComboBoxModel<RecentFile> {
   ...
   private JTextPane createReportPage(File file)
         throws IOException {
   ...
   private static class PopupMenuMouseAdapter
      extends
      MouseAdapter {
   ...
   public synchronized void appendHTML(String html) {
      ...
      if(mStartUp != null)
         ...
         try {
            ...
            try (final Writer out = new OutputStreamWriter(new FileOutputStream(DTSA2.getReport().getFile()), java.nio.charset.Charset.forName("ISO-8859-1"))) {
   ...
   private void otherInit() {
      ...
      class SpectrumSelectionListener
         implements
   ...
   private void initialize()
      ...
      addUtilityTab("Report", tab, "An HTML summary of this DTSA-II session");
      {
         ...
         jTabbedPane_Utility.addTab("Command", ii, jPanel_Command, "A command line for controlling DTSA-II in Python");
      }
      /*
            ...
            try {
               ...
               if(dtde.getTransferable().isDataFlavorSupported(flavor)) {
                  ...
                  if(data instanceof List<?>) {
                     ...
                     if(failed.size() > 0) {
                        ErrorDialog.createErrorMessage(MainFrame.this, "Drag-and-Drop", "Unable to open " + failed.size()
                              + " dropped files.", failed.toString());
                     }
                  }
               }
            }
   ...
   public static File selectFileDestination(String fn, Component parent) {
      ...
      jfc.addChoosableFileFilter(new SimpleFileFilter(new String[] {
         "tia.msa"
      }, "TIA Compatible EMSA/MAS Standard"));
      jfc.addChoosableFileFilter(new SimpleFileFilter(new String[] {
         "tif",
         "tiff"
      }, "ASPEX-style TIFF Spectrum"));
      final SimpleFileFilter emsa = new SimpleFileFilter(new String[] {
         "msa",
         "emsa",
         "txt"
   ...
   public void openSpectrumFile(File f)
         throws EPQException {
         ...
         for(int j = 0; j < specs.length; ++j) {
            ...
            if(det != null) {
               ...
               if(dc instanceof EDSCalibration) {
                  ...
                  if(!dontApplyToAll) {
                     if(!assign) {
                        final int opt = JOptionPane.showConfirmDialog(MainFrame.this, "<html>The calibration of <i>"
                              + sd.toString() + "</i><br>" + "does not seem to be similar to the default detector.<br><br>"
                              + "Apply the default detector none the less?", "Spectrum open", JOptionPane.YES_NO_CANCEL_OPTION);
                        if(opt == JOptionPane.CANCEL_OPTION) {
                           if((j == 0) && (specs.length > 1))
                              if(JOptionPane.showConfirmDialog(MainFrame.this, "Cancel all " + Integer.toString(specs.length)
                                    + " spectra?", "File Open", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                                 break;
                           continue;
                        }
                        if(opt == JOptionPane.NO_OPTION)
                           if((j == 0) && (specs.length > 1))
                              dontApplyToAll = JOptionPane.showConfirmDialog(MainFrame.this, "Don't apply the default detector to all "
                                    + Integer.toString(specs.length)
                                    + " spectra?", "File Open", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION;
   ...
   private ISpectrumData readUCalFile(File file)
         throws EPQException {
   ...
   public void fileImport() {
      ...
      final SimpleFileFilter csvFF = new SimpleFileFilter(new String[] {
         "csv"
      }, "CSV Files");
      final SimpleFileFilter edaxFF = new SimpleFileFilter(new String[] {
         "csv"
      }, "EDAX CSV files (10 eV/ch)");
      final SimpleFileFilter ucalFF = new SimpleFileFilter(new String[] {
         "txt"
      }, "uCal TSV files (1 eV/ch)");
      ...
      if(fc.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
         ...
         for(final File file : files)
            if(fc.getFileFilter() == ucalFF)
               ...
               final EditableSpectrum es = new EditableSpectrum(tbl.length, mult * w0, mult * tbl[0][0]);
   ...
   private static class RenameDialog
      extends
      JDialog {
               ...
               if(specs.size() > 0) {
                  ...
                  if(msd.shouldSave()) {
                     ...
                     if(res != null) {
                        ...
                        if(spectra.size() > 1) {
                           hl.append("<p><table>\n");
                           hl.append("<tr><th>Spectrum</th><th>Beam Energy<br>(keV)</th><th>Probe Current<br>(nA)</th><th>Live Time<br>(s)</th><th>Counts/(nA\u00B7S)</th><th>Variance<br>(&sigma;)</th></tr>\n");
                           for(final ISpectrumData spec : spectra) {
                              final SpectrumProperties sp = spec.getProperties();
                              hl.append("<tr>");
                              hl.append("<td>" + spec.toString() + "</td>");
                              hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.BeamEnergy, Double.NaN))
                                    + "</td>");
                              hl.append("<td>" + hu3.format(SpectrumUtils.getAverageFaradayCurrent(sp, Double.NaN)) + "</td>");
                              hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN))
                                    + "</td>");
                              final UncertainValue2 uv2 = UncertainValue2.divide(UncertainValue2.createGaussian(SpectrumUtils.totalCounts(spec, true), spec.toString()), SpectrumUtils.getDose(sp));
                              hl.append("<td>" + uv2.format(hu) + "</td>");
                              final UncertainValue2 tmp = UncertainValue2.divide(uv2, cpnas.average());
                              hl.append("<td>" + hu.format((1.0 - tmp.doubleValue()) / tmp.uncertainty()) + "</td>");
                              hl.append("</tr>\n");
                           }
                           hl.append("</table></p>");
                           hl.append("<p><table>\n");
                        }
                        hl.append("<tr><th>Spectrum</th><th>Beam Energy<br>(keV)</th><th>Probe Current<br>(nA)</th><th>Live Time<br>(s)</th><th>Counts/(nA\u00B7S)</th></tr>\n");
                        {
                           ...
                           hl.append("<td>" + hu.format(sp.getNumericWithDefault(SpectrumProperties.LiveTime, Double.NaN))
                                 + "</td>");
                           hl.append("<td>" + hu.format(SpectrumUtils.totalCounts(res, true) / SpectrumUtils.getDose(sp))
                                 + "</td>");
                           hl.append("</tr>\n");
                        }
                        hl.append("</table></p>");
   ...
   public void fileSaveAs() {
      ...
      if(list.size() > 0) {
         appendHTML(list.toString());
         if(errCx > 0)
            ErrorDialog.createErrorMessage(this, "File Save Errors", Integer.toString(errCx)
                  + (errCx == 1 ? " spectrum" : " spectra") + " not written successfully.", "<html>" + list.toString());
      }
   }
   ...
   public File saveStandard(Element elm, StandardBundle bundle) {
      ...
      final String dir = DTSA2.getSpectrumDirectory();
      final JFileChooser jfc = new JFileChooser(dir);
      final SimpleFileFilter std = new SimpleFileFilter(new String[] {
         "zstd"
   ...
   public static String exportSpectrumAsCSV(ISpectrumData sd, String filename)
         throws Exception {
   ...
   public static String exportSpectrumAsEMSA(ISpectrumData sd, String filename)
         throws Exception {
   ...
   public static String exportSpectrumAsTIFF(ISpectrumData sd, String filename)
         throws Exception {
      final String fu = filename.toUpperCase();
   ...
   public static String exportSpectrumAsTiaEMSA(ISpectrumData sd, String filename)
         throws Exception {
      final String fu = filename.toUpperCase();
         ...
         try {
            ...
            if(comp != null) {
               ...
               for(int i = r.size() - 1; i >= 0; --i) {
                  ...
                  rois.add(new int[] {
                     SpectrumUtils.channelForEnergy(sd, rr.getLowEnergy()),
                     SpectrumUtils.channelForEnergy(sd, rr.getHighEnergy())
                  });
               }
               if(rois.size() > 0) {
                  final ISpectrumData bkg = ba.fitBackground(det, sd, rois);
                  bkg.getProperties().setTextProperty(SpectrumProperties.SpectrumDisplayName, "Brem[" + sd.toString()
                        + ", manual rois]");
