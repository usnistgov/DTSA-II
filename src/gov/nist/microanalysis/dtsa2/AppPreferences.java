...
public class AppPreferences {
   ...
   private boolean mOFudge = false;
	...
	/**
	 * Default tolerance for assuming that two spectra are calibrated the same. Use
	 * with SpectrumUtils.areCalibratedSimilar.
	 */
	...
	private class BasicPreferences extends PreferencePanel {
		...
		private void initialize() {
			final FormLayout fl = new FormLayout("5dlu, right:pref, 5dlu, 150dlu, 3dlu, pref, 3dlu, pref",
					"pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref, 5dlu, pref, 3dlu, pref");
		...
		public void commit() {
			...
			setAppearance((Appearance) jCombo_LaF.getSelectedItem());
			HTMLReport.setBasePath(jTextField_ReportPath.getText());
		}

	}

	private class QuantPreferences extends PreferencePanel {
	...
	private class InstrumentPanel extends PreferencePanel {
		...
		private boolean checkDetectorGUID(EDSDetector det) {
			...
			for (final DetectorProperties other : allDetectors) {
				final String otherGuid = (String) other.getProperties()
						.getObjectWithDefault(SpectrumProperties.DetectorGUID, null);
		...
		private void initialize() {
			setLayout(new FormLayout("5dlu, right:pref, 5dlu, 30dlu, 30dlu, right:pref, 5dlu, 30dlu",
					"20dlu, pref, 10dlu, pref, 20dlu, pref"));
			final CellConstraints cc = new CellConstraints();
			...
			jTextField_Name.addFocusListener(new FocusAdapter() {
				...
				public void focusLost(FocusEvent e) {
					...
					if (newName.length() > 0) {
						final String name = mInstrument.getProbeProperties()
								.getTextWithDefault(SpectrumProperties.Instrument, null);
			...
			imprt.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						...
						final SimpleFileFilter xdet = new SimpleFileFilter(new String[] { "xdet" },
			...
			// Set data fields
			jTextField_Name
					.setText(mInstrument.getProbeProperties().getTextWithDefault(SpectrumProperties.Instrument, "?"));
	...
	private class AddInstrument extends PreferencePanel {
		...
		public void initialize() {
			...
			help.setText("<HTML><h3 align=right>Overview</h3><p align=left>" + DTSA2.APP_NAME
					...
					+ "of the instrument.  From the instrument page you can edit existing "
					+ "detectors or add new ones." + "</p>");
	...
	private class SiLiPanel extends DetectorPanel {
		...
		private void initialize() {
					...
					if (specs != null) {
						...
						if (props.isDefined(SpectrumProperties.DetectorDescription)) {
							jTextField_Name.setText(
									props.getTextWithDefault(SpectrumProperties.DetectorDescription, "Detector"));
							jTextField_Name.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.DetectorGUID)) {
							jTextField_GUID
									.setText(props.getTextWithDefault(SpectrumProperties.DetectorGUID, AUTO_GENERATE));
							jTextField_GUID.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.Elevation)) {
							jTextField_Elevation
									.setValue(props.getNumericWithDefault(SpectrumProperties.Elevation, 35.0));
						...
						if (props.isDefined(SpectrumProperties.WorkingDistance)) {
							jTextField_OptWD
									.setValue(props.getNumericWithDefault(SpectrumProperties.WorkingDistance, 20.0));
						...
						if (props.isDefined(SpectrumProperties.DetectorArea)) {
							jTextField_Area
									.setValue(props.getNumericWithDefault(SpectrumProperties.DetectorArea, 10.0));
						...
						if (props.isDefined(SpectrumProperties.AluminumLayer)) {
							jTextField_AlLayer
									.setValue(props.getNumericWithDefault(SpectrumProperties.AluminumLayer, 0.0));
							jTextField_AlLayer.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.NickelLayer)) {
							jTextField_NiLayer
									.setValue(props.getNumericWithDefault(SpectrumProperties.NickelLayer, 0.0));
							jTextField_NiLayer.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.DeadLayer)) {
							jTextField_DeadLayer
									.setValue(props.getNumericWithDefault(SpectrumProperties.DeadLayer, 0.10));
							jTextField_DeadLayer.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.ActiveLayer)) {
							final double thickness = props.getNumericWithDefault(SpectrumProperties.ActiveLayer, 5000.0)
									/ 1000.0;
						...
						if (props.isDefined(SpectrumProperties.ZeroPeakDiscriminator)) {
							jTextField_ZPD
									.setValue(props.getNumericWithDefault(SpectrumProperties.ZeroPeakDiscriminator, 0));
							jTextField_ZPD.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.EnergyScale)) {
							jTextField_Scale
									.setValue(props.getNumericWithDefault(SpectrumProperties.EnergyScale, 10.0));
							jTextField_Scale.setBackground(updatedColor);
						}
						if (props.isDefined(SpectrumProperties.EnergyOffset)) {
							jTextField_Offset
									.setValue(props.getNumericWithDefault(SpectrumProperties.EnergyOffset, 0.0));
			...
			final PanelBuilder location = new PanelBuilder(
					new FormLayout(xFmt, "pref, 5dlu, pref, 5dlu, pref, 5dlu, pref, 2dlu, pref, 2dlu"));
			...
			final PanelBuilder performance = new PanelBuilder(
					new FormLayout(xFmt, "pref, 5dlu, pref, 5dlu, pref, 2dlu"));
			...
			perfPanel.setToolTipText(
					"<html>These properties are not editable for established detectors.<br/>Use the calibration alien to refine them.");
...

			{
				final ButtonBarBuilder bbb = new ButtonBarBuilder();
				bbb.addGlue();
				jButton_Export.setToolTipText(
						"Export the detector definition as XML to facilitate sharing this information with colleagues.");
			...
			jButton_Export.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					jfc.setMultiSelectionEnabled(false);
					jfc.setAcceptAllFileFilterUsed(true);
					final SimpleFileFilter sff = new SimpleFileFilter(new String[] { "xdet", },
					...
					try {
						...
						ErrorDialog.createErrorMessage(SiLiPanel.this.getPreferenceDialog(), "Error saving detector",
								e);
					}
				}
			});
		...
		public EDSDetector getModified() throws EPQException {
			...
			if (isModified() || isNewDetector()) {
				...
				sp.setDetectorPosition(Math.toRadians(elevation), Math.toRadians(azimuth), 1.0e-3 * distance,
						1.0e-3 * optWd);
				...
				// Window paramters
				final IXRayWindowProperties newWind = XRayWindowFactory
						.createWindow((String) jComboBox_Window.getSelectedItem());
				...
				if (isNewDetector()) {
					...
					ec = new SDDCalibration(jTextField_Scale.getValue(), jTextField_Offset.getValue(),
							jTextField_Resolution.getValue());
	...
	private class MicrocalPanel extends DetectorPanel {
			...
			jButton_Export.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					try {
						...
						ErrorDialog.createErrorMessage(MicrocalPanel.this.getPreferenceDialog(),
								"Error saving detector", e);
					}
				}
			});
		...
		public EDSDetector getModified() throws EPQException {
			...
			if (isModified() || isNewDetector()) {
				...
				if (isNewDetector()) {
					dp.setChannelCount(jTextField_NChannels.getValue());
					ec = new MicrocalCalibration(jTextField_Scale.getValue(), jTextField_Offset.getValue(),
							jTextField_Resolution.getValue());
					ec.makeBaseCalibration();
				} else
					ec = mOriginal.getCalibration();
	...
	private AppPreferences() {
		...
		mBremAngular = userPref.get("Bremsstrahlung angular distribution",
				BremsstrahlungAngularDistribution.Acosta2002L.getName());
		mIonizationXSec = userPref.get("Ionization cross section",
	...
	private void updateStrategy() {
		...
		{
			final AlgorithmClass ac = lookUp(CorrectionAlgorithm.NullCorrection.getAllImplementations(),
					mCorrectionAlgorithm);
			if (ac != null)
		...
		{
			final AlgorithmClass ac = lookUp(AbsoluteIonizationCrossSection.BoteSalvat2008.getAllImplementations(),
					mIonizationXSec);
			if (ac != null)
				strat.addAlgorithm(AbsoluteIonizationCrossSection.class, ac);
		}
		{
			final AlgorithmClass ac = lookUp(BremsstrahlungAngularDistribution.Acosta2002.getAllImplementations(),
					mBremAngular);
			if (ac != null)
